...
2. Install apps, create projects and import dandisets
3. Run Dendro jobs
4. Visualize results on Neurosift
...
<details>
<summary>1. Start an instance on [dandihub](https://hub.dandiarchive.org/). We suggest a `Medium` instance, which has 12 CPUs and 32GB of memory.</summary>
...
## Install apps, create projects and import dandisets

With a Compute Resource up and running, you can now install apps, create projects and import dandisets to start running Dendro jobs.

Let's start by installing apps to your Computer Resource, through Dendro UI:
...
Now, let's create a project and import a dandiset to it.
...
- Click on `Create a new project`, choose a name for it, e.g. `dendro-tutorial-project`
...
Import a dandiset to your Dendro project:
- At the left side navigation menu, click on `DANDI Import`
- Add the dandiset ID `000409`
